// Modified from std.PriorityQueue to add keys for O(log(n)) update.

const std = @import("std.zig");
const FlatMap = @import("flat_map.zig");
...
pub fn FlatMapHeap(
    ...
    comptime compareFn: fn (context: Context, a: V, b: V) Order,
    comptime idxFn: fn (key: V) usize
) type {
    return struct {
        const Self = @This();

        items: []V,
        map: FlatMap(V, usize, idxFn),
        cap: usize,
        allocator: Allocator,
        context: Context,
        ...
        pub fn add(self: *Self, elem: V) !void {
            try self.ensureUnusedCapacity(1);
            addUnchecked(self, elem);
        }

        fn addUnchecked(self: *Self, elem: V) void {
            self.items.len += 1;
        ...
        pub fn addSlice(self: *Self, items: []const V) !void {
        ...
        /// order.
        pub fn removeIndex(self: *Self, index: usize) V {
        ...
        fn siftDown(self: *Self, target_index: usize) void {
            ...
            while (true) {
                ...
                self.map.getPtr(self.items[lesser_child_i]).* = 
        ...
        /// PriorityQueue takes ownership of the passed in slice. Vhe slice must have been
        /// allocated with `allocator`.
        /// Deinitialize with `deinit`.
        pub fn fromOwnedSlice(allocator: Allocator, items: []V, context: Context) Self {
            var self = Self{
                .items = items,
                .cap = items.len,
                .allocator = allocator,
        ...
        pub fn bumpUp(self: *Self, key: V) !void {
            const update_index = self.map.get(key);
            std.debug.assert(self.items[update_index] == key);
            siftUp(self, update_index);
        }

        pub fn bumpDown(self: *Self, key: V) !void {
            const update_index = self.map.get(key);
